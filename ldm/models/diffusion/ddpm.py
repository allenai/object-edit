...
# from ldm.modules.lora import inject_trainable_lora, extract_lora_ups_down
        ...
        if isinstance(x,dict):
            ...
            for key,tensor in x.items():
                if isinstance(tensor,torch.Tensor):
                    ...
                    tensor = tensor.to(memory_format=torch.contiguous_format).float()
                    x[key] = tensor
            return x
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if isinstance(xc,dict):
            for k,v in xc.items():
                if isinstance(v,torch.Tensor):
                    xc[k] = v.to(self.device)
        else:
            xc = xc.to(self.device)
        if bs is not None:
            if isinstance(xc,dict):
                xc["image"] = xc["image"][:bs]
                xc["text"] = xc["text"][:bs]
            else:
                xc = xc[:bs]
        cond = {}
        ...
        with torch.enable_grad():
            ...
            cond["c_crossattn"] = [self.cc_projection(torch.cat([torch.where(prompt_mask, null_prompt, clip_emb), T[:, None, :].repeat(1,clip_emb.shape[1],1)], dim=-1))]
        if isinstance(xc,dict):
            cond["c_concat"] = [input_mask * self.encode_first_stage((xc["image"].to(self.device))).mode().detach()]
        else:
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                ...
                log['conditioning'] = xc
            elif self.cond_stage_key == 'cond':
                xc_text = log_txt_as_img((x.shape[2],x.shape[3]),batch["cond"]["text"],size=x.shape[2]//25)
                xc_img = batch["cond"]["image"]
                log["conditioning_text"] = xc_text
                log["conditioning_image"] = xc_img
            elif isimage(xc):
                log["conditioning"] = xc
            if ismap(xc):
    ...
    def configure_optimizers(self):
        ...
        if self.use_lora: # NOTE: I don't think this works as intended. 1447 should be uncommented but gets error

            # self.model.diffusion_model.requires_grad_(False)
            image_encoder = self.cond_stage_model.model
            unet_lora_params, train_names = inject_trainable_lora(self.model.diffusion_model)
            # import ipdb; ipdb.set_trace()
            unet_lora_params = list(itertools.chain(*unet_lora_params,image_encoder.parameters()))
            opt = torch.optim.AdamW([{"params": unet_lora_params , "lr": lr},
