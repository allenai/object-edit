...
class FrozenCLIPImageEmbedder(AbstractEncoder):
    ...
    def __init__(
        ...
        self.text_model = FrozenCLIPEmbedder(device=device)
    ...
    def forward(self, x):
        if isinstance(x,dict):
            encoded_images = self.model.encode_image(self.preprocess(x["image"])).float().unsqueeze(1)
            encoded_text = self.text_model(x["text"]).float()
            return torch.cat([encoded_text,encoded_images],dim=1)
        else:
                # x is assumed to be in range [-1,1]
                if isinstance(x, list):
                    # [""] denotes condition dropout for ucg
                    device = self.model.visual.conv1.weight.device
                    return torch.zeros(1, 768, device=device).unsqueeze(1)
                    
                return self.model.encode_image(self.preprocess(x)).float().unsqueeze(1)
