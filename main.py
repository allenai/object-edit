...
class ImageLogger(Callback):
    ...
    @rank_zero_only
    def _wandb(self,pl_module,images,batch_idx, log_key):
               
        with torch.no_grad():
            target = images['inputs']
            pred = images['samples']
            mse = torch.nn.functional.mse_loss(target,pred)
            pl_module.log(f"{log_key}_pred_mse",mse,on_step=True,on_epoch=False)

            masks = images["mask_target"]
            pm = 0.0
            total = 0
            for i in range(len(target)):
                try:
                    metric = psnr_mask(pred[i].unsqueeze(0),target[i].unsqueeze(0),masks[i])
                    pm += metric
                    total += 1
            ...
            pm /= total
            pl_module.log(f"{log_key}_psnr_mask",pm,on_step=True,on_epoch=False)
    ...
    def log_local(self, save_dir, split, images,
        ...
        cat_keys = ["conditioning_text","conditioning_image","inputs","samples"]

        img = torch.cat([images[k].cpu() for k in cat_keys],dim=-1)
        if self.rescale:
        ...
        last_group_size = B % group_size

        for i in range(num_groups):
            group_start = i * group_size
            group_end = (i + 1) * group_size
            group_images = img[group_start:group_end].reshape(-1,C,H,W)
            filename = "{}/gs-{:06}_e-{:06}_b-{:06}_{:06}.png".format(root,global_step,current_epoch,batch_idx,i+1)
            torchvision.utils.save_image(group_images, filename, nrow=1, padding=0, normalize=True)

        if last_group_size > 0:
            group_start = num_groups * group_size
            group_images = img[group_start:].reshape(-1,C,H,W)
            filename = "{}/gs-{:06}_e-{:06}_b-{:06}_{:06}.png".format(root,global_step,current_epoch,batch_idx,num_groups+1)
            torchvision.utils.save_image(group_images, filename, nrow=1, padding=0, normalize=True)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if ((should_log or  check_idx % self.batch_freq == 0) and
            ...
            with torch.no_grad():
                ...
                images["mask_target"] = batch["mask_target"]
            for k in images:
            ...
            group_by_log_key = defaultdict(dict) # need to group by logkey if task is multitask
            B, _, _, _ =  images["inputs"].shape
            for i in range(B):
                log_key = batch["log_key"][i]
                for img_key in images.keys():
                    if not img_key in group_by_log_key[log_key]:
                        group_by_log_key[log_key][img_key] = []
                    group_by_log_key[log_key][img_key].append(images[img_key][i])

            for lk in group_by_log_key:
                for imk in group_by_log_key[lk]:
                    group_by_log_key[lk][imk] = torch.stack(group_by_log_key[lk][imk])
            
            for k in group_by_log_key:
                self.log_local(pl_module.logger.save_dir, k, group_by_log_key[k],
                            pl_module.global_step, pl_module.current_epoch, batch_idx)

                logger_log_images = self.logger_log_images.get(logger, lambda *args, **kwargs: None)
                logger_log_images(pl_module, group_by_log_key[k], pl_module.global_step, k)
